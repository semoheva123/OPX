...
async function getVaultContracts(req, res) {
  try {
    const storedContracts = await InvestmentVaultContract.find({ enabled: true }).sort({ durationDays: 1 }).lean();
...
async function getVaults(req, res) {
  try {
    const vaults = await InvestmentVault.find({ userId: req.user.id }).sort({ createdAt: -1 }).lean();
    const now = Date.now();
