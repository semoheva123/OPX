...
async function createInvestmentVault(event) {
    ...
    const confirmed = await showPlatformConfirm(`سيتم تجميد ${amount.toFixed(2)} USDT لمدة ${durationDays} يومًا. لا يمكن الاسترداد قبل الاستحقاق. هل تتابع؟`, 'تأكيد تجميد السيولة');
    if (!confirmed) return;
    try {
...
async function loadTeamReferrals() {
    ...
    try {
        ...
        list.innerHTML = data.referrals?.length ? data.referrals.map(member => { const active = !member.isBanned && Number(member.wallet?.totalDeposits || 0) > 0; return `<div class="team-member flex items-center justify-between gap-3 rounded-xl p-3"><div><b class="block text-xs text-slate-200">${escapeAiHtml(member.email)}</b><span class="text-[10px] text-slate-500">${active ? `مستوى ${escapeAiHtml(member.tierCode || 'A1')}` : 'غير مفعّل'} • ${new Date(member.createdAt).toLocaleDateString('ar')}</span></div><span class="text-[10px] font-bold ${active ? 'text-emerald-300' : 'text-slate-500'}">${active ? 'نشطة' : member.isBanned ? 'محظورة' : 'غير مفعلة'}</span></div>`; }).join('') : '<p class="text-[11px] text-slate-500 text-center py-4">لم تسجل إحالات مباشرة بعد.</p>';
